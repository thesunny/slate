...
export default function setTextFromDOMSelection(target, change, editor, src) {
  // debug('onTextChange', { event })
  console.warn('setTextFromDOMSelection', src)
  ...
  const domPoint = findDomPoint(point, window)
  const domNode = findDomNode(node.key, window)
  console.log('dump', point.toJS(), 'DOMPOINT', domPoint, domNode, anchorNode)
  ...
  let { textContent: nextText } = anchorNode
  ...
  setSelectionFromDOM(target, change, editor, { from: 'onTextChange' })
  console.log(
